...
#   Thom Volker
#   13-05-2022
#   Simulation 3 in Volker & Klugkist "Combining support for hypotheses over 
...
#   predictors, and we evaluate the hypothesis beta_6 > 0, which corresponds 
#   to V6 > 0 in the code
#
# ______________________________________________________________________________
...
output <- 
  expand_grid(nsim = 1:nsim,                                     # 1. data.frame with all
  ...
  mutate(rho   = map(pcor, ~cormat(.x, length(ratio_beta))),     # 2. create covariance matrix
         ...
         fit  = future_pmap(list(r2, betas, rho, n, model),      # 4. generate normal, logit and 
                            function(r2, betas, rho, n, model) { #    probit data, fit full
...
save.image(file = "output/03_dummied_variable_cont.RData")
